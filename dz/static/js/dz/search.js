...
// 更新房源列表信息
// action 表示从后端请求的数据在前端的展示方式
// 默认采用追加方式
// action=new 代表页面上的数据清空重新展示
    ...
    $.get("/api/houses", params, function (resp) {
        ...
        if(resp.errno=="0"){
            if (resp.data.total_page == 0) {
                ...
                if ("renew" == action) {
                    cur_page = 1;
                    $(".house-list").html(template("house-list-temp", {houses: resp.data.houses}));
                } else {
                    cur_page = next_page;
                    $(".house-list").append(template("house-list-temp", {houses: resp.data.houses}));
                }
            }
        }
    })
            ...
            if (areaId) {
                for (var i=0; i<resp.data.length; i++) {
                    ...
                    if (resp.data[i].aid == areaId) {
                        $(".filter-area").append('<li area-id="'+ resp.data[i].aid+'" class="active">'+ resp.data[i].a_name+'</li>');
                    } else {
                        $(".filter-area").append('<li area-id="'+ resp.data[i].aid+'">'+ resp.data[i].a_name+'</li>');
                    }
                }
            }else {
                for (var i=0; i<resp.data.length; i++) {
